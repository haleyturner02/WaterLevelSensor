...
//char packet[] = {0x12, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A};     // 6 byte packet
...
void initTimerB1compare() {     // Setup TB1
...
void getWaterLevel() {
    ...
    if(avg >= 0 && avg <= 10) {                 // Empty
        key = 0x0B;
        if(enableLED == 1) {
            P2OUT |= BIT0;
        }
    } else if(avg > 10 && avg <= 200) {         // Low
        key = 0x0C;
        if(enableLED == 1) {
            P2OUT |= BIT1;
        }
    } else if(avg > 200 && avg <= 325) {        // Good
        key = 0x0D;
        if(enableLED == 1) {
            P2OUT |= BIT2;
        }
    } else if(avg > 325 && avg <= 400) {        // High
        key = 0x0E;
        if(enableLED == 1) {
            P2OUT |= BIT3;
        }
    } else if(avg > 400){                       // Full
        key = 0x0F;
        if(enableLED == 1) {
            P4OUT |= BIT0;
        }
...
int main(void) {
    ...
    initLEDs();                         // Intialize indicator LEDs
...
void displayUnlockPattern() {           // Pattern to indicate system has been successfully unlocked
    int i;
    initLEDs();
    for(i = 0; i <= 32000; i++) {}
    P2OUT |= (BIT0 | BIT1 | BIT2 | BIT3);
...
void keyPressedAction(char pressed_key) {
    if(is_unlocked(pressed_key) == 1) {
        switch(pressed_key) {                   // Get pressed key
            ...
            case 0x80:                  // A- Relock system
                relock();
                break;
            case 0x40:                  // B- Enable/disable LEDs
                if(enableLED == 1) {
                    enableLED = 0;
                } else {
                    enableLED = 1;
